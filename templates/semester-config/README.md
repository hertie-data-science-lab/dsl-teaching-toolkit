<!-- SYSTEM-OWNED - do not edit, edits here are overwritten. This README, and the workflows under .github/, are regenerated by every "Bootstrap semester" run (cron-run as part of the nightly Refresh), so keep notes elsewhere. Every OTHER non-sample file in this repo is INSTRUCTOR-OWNED: the engine never rewrites students.csv, schedule.yml, instructors.yml, teams.csv, or anything you type into a grading sheet, once they exist. -->
...
and this semester's `schedule.yml`. Edit them **there** - the sheet's copy is regenerated on
every write.
