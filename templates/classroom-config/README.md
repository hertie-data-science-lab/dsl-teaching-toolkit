<!-- SYSTEM-OWNED - do not edit, edits here are overwritten. This README, and the workflows under .github/, are regenerated by every "Bootstrap cohort" run (cron-run as part of the nightly Refresh), so keep notes elsewhere. Every OTHER non-sample file in this repo is INSTRUCTOR-OWNED: the engine never rewrites students.csv, schedule.yml, people.yml, teams.csv, or anything you type into a grading sheet, once they exist - except when you re-point a student's `github_handle` at a new GitHub account, which moves that student's rows with them. -->
