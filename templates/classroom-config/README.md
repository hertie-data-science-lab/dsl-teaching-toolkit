<!-- SYSTEM-OWNED - do not edit, edits here are overwritten. This README, and the workflows under .github/, are regenerated by every "Bootstrap semester" run (cron-run as part of the nightly Refresh), so keep notes elsewhere. Every OTHER non-sample file in this repo is INSTRUCTOR-OWNED: the engine never rewrites students.csv, schedule.yml, people.yml, teams.csv, or anything you type into a grading sheet, once they exist. -->

# classroom-config - this semester's private config
...
## `people.yml` - this semester's instructors/TAs (optional)
...
**Sync membership** reconciles them into this semester's own `instructors` team AND a course-org level `instructors-<tag>` team (push access scoped to just this year's content repos, plus the central `.github` repo so they can use the workflows too). It also supplies this semester's website staff cards. 
