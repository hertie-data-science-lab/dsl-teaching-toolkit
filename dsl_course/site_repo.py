...
What a semester site and a public course site SHARE: the pages and nav the theme renders,
...
what they link to) is the caller's: `site` builds a semester site's plan, `public_site` the
...
def site_readme(org: str, semester: bool) -> str:
    ...
    tab_pages = ", ".join(f"`{pg.file}`" for pg in _site_pages(semester))
    source = (
        "the semester's `semester-config/` files (`schedule.yml`, `instructors.yml`) and what "
...
class _ThemePage:
    ...
    assignments page names students alone. `gated_note` is what a SEMESTER site says;
...
def yaml_file(org: str, repo: str, path: str) -> dict:
    ...
    here (via load_yaml_config), to the per-semester isolation the callers already have. It
    used to be coerced to `{}`, so one bad indent in a semester's instructors.yml republished the
...
def people_cards(
    ...
    there) or a semester's own `semester-config/instructors.yml` (semester site: instructors
    ...
    people-cards.yml for the course org's block, templates/semester-config/instructors.yml
    for a semester's):
...
def people_yaml(
    ...
    *,
    ...
    (the course org's dsl-course.yml for the course site, a semester's semester-config/
    instructors.yml for the semester site); else fall back to the GitHub `instructors` team of
    ...
    `include_tas=False` (the course site) drops TA cards entirely - TAs are semester-only,
    ...
    featured = instructors[0] if instructors else {"name": "Course instructors"}
    return (
...
def iso_when(when: date | datetime, fallback_time: str = "09:00:00") -> str:
    ...
    A datetime from schedule.yml is ALREADY in the semester timezone - the parser converts
    an entry written with an explicit offset (`...T10:00+00:00`) into the semester's own
...
def _last_touched_before_head(wd: Path) -> dict[str, tuple[str, str, str]]:
    ...
    generated page and collection entry, so an ordinary semester site paid a hundred-odd
...
def _notify_overwritten_edits(
    ...
    body = (
        ...
        "- **Instructor cards** - the semester's `semester-config/instructors.yml` (for a public "
        "course site, the `people:` block of the course org's "
        "`.github/dsl-course.yml`).\n"
        "- **Schedule rows, sessions, assignments** - the org structure and the semester's "
    ...
    # instructors.yml to address, and a semester whose transport is unset gets the issue alone.
    # `course_org_for_semester` is "" for the course org itself, which IS the course org.
    notify.notify_overwritten_edits(
        org,
        site,
        course_org_for_semester(org) or org,
        by_login,
        upserted.url,
        datetime.now(UTC),
    )
...
def _stale_site_repo(org: str, site: str) -> str | None:
    ...
    site repo is then simply absent, which every sync happily read as "this semester
...
def sync_site_repo(
    ...
    *,
    ...
    repo is a quiet no-op (a semester that never opted into a site), unless
    `scaffold_missing` - the public course site's opt-in first publish, which creates it.
    An ARCHIVED one is the same quiet no-op: a past semester's site is deliberately frozen,
