...
60 days). Archiving is opt-in, so the Archive semester button is for closing one out early,
and the only way to close out a semester that wrote no block. Six steps, in this order, and
the order is the whole design:

0. PROPAGATE: offer the semester's edits to its released material back to the course org as
...
def archive_notice_title(when: date) -> str:
    """The title of the "this semester is about to freeze" issue.
...
def is_archive_notice(title: str) -> bool:
    ...
    notice whose date has been moved or taken away while the semester is still live
...
def archive_due(sched: schedule.Schedule, today: date) -> bool:
    """Whether this semester's archive date has arrived.

    A semester that writes no `archive:` block has no such date and is never due: archiving
...
def registrar_summary(semester_org: str) -> str:
    ...
    text = get_file_content(semester_org, CONFIG_REPO, SEMESTER_CSV_NAME)
    if text is None:
        return f"{SEMESTER_CSV_NAME} is NOT here - no grade was ever sent"
    rows = len(list(read_csv(text, (), SEMESTER_CSV_NAME)))
    return f"{SEMESTER_CSV_NAME}, {rows} student row(s)"
...
def _carry_back(course_org: str, semester_org: str, dry_run: bool) -> tuple[str, int]:
    """Step 0: offer this semester's edits back to the course org. `(what the record says,
    errors)`.

    Nothing here can stop the semester being sealed. A course org that never wanted the
    semester's edits is still entitled to a closed semester, and leaving one half-frozen
    ...
    A semester repo the propagate did not READ is the one outcome that is neither an error
    ...
    `Propagated(0, ())` and this wrote "nothing had been edited" over a semester whose
    ...
    if not course_org:
        log(
            "  [skip] no --course-org given, so this semester's edits are NOT being "
            "carried back. Run Propagate semester edits before un-archiving anything."
        )
        return "not offered - no course org was named", 0
    try:
        done = propagate.propagate(course_org, semester_org, dry_run=dry_run)
    except Exception as exc:
        log_err(
            f"could not carry {semester_org}'s edits back to {course_org} "
            f"({type(exc).__name__}): {exc}"
        )
        return "FAILED - the semester was sealed without carrying them back", 1
    said: list[str] = []
    if done.errors:
    ...
    if done.behind:
        said.append(
            ...
            f"Propagate semester edits before un-archiving anything"
        )
...
def _final_sync(course_org: str, semester_org: str, dry_run: bool) -> int:
    """Step 2: one last website sync, before the site repo is frozen.

    The deployed site carries the semester's own "Semester archived" schedule row
    ...
    not a reason to leave a semester half-closed."""
    if not course_org:
        return 0
    if dry_run:
    ...
    try:
        if site.sync_site(course_org, semester_org) != 0:
            log_err(f"{semester_org}'s final website sync was incomplete")
            return 1
    except Exception as exc:
        log_err(f"{semester_org}'s final website sync failed: {exc}")
        return 1
    return 0
...
def _freeze(semester_org: str, listing: list[dict], dry_run: bool) -> Closed:
    ...
    already: list[str] = []
    errors = 0
    for repo in freeze_order(semester_org, listing):
        name = repo["name"]
...
def close_out(
    ...
    """Close `semester_org` out. 0 when it is closed (or already was), 1 on any failure.

    `course_org` may be `""`: the semester is still closed, but its edits are not offered
    ...
    where it has one: the scheduler decides the semester is due off its own `now` (which
    ...
    if config is None:
        log_err(
            f"{semester_org} has no {CONFIG_REPO} - it is not a bootstrapped semester org, "
    ...
    if not archive_due(sched, today or datetime.now(timezone.utc).date()) and not force:
        ...
        not_due = (
            f"{semester_org} is not due to be archived: {CONFIG_REPO}/"
    ...
    registrar = registrar_summary(semester_org)
    log(f"  registrar export: {registrar}")
    propagated, errors = _carry_back(course_org, semester_org, dry_run)
    errors += _close_notices(semester_org, dry_run)
    errors += _final_sync(course_org, semester_org, dry_run)
    ...
    if dry_run:
        ...
        return Summary(
            ...
            f"({len(closed.already)} already are), then the semester is archived.",
    ...
    if not put_file(
        ...
        return 1
    if not archive_repo(semester_org, CONFIG_REPO):
        log_err(
            f"the record is written but {semester_org}/{CONFIG_REPO} is NOT sealed, so "
            f"every nightly sweep still treats this semester as live. Re-run to finish."
        )
        return 1
    log_ok(
    ...
    return Summary(
        f"Semester archived: {plural(total, 'repo')} read-only, nothing deleted.",
        {"repos": total},
    )
