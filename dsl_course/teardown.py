...
def _carry_back(course_org: str, cohort_org: str, dry_run: bool) -> tuple[str, int]:
    ...
    if done.errors:
        return f"{done.errors} path(s) could not be carried back", done.errors
    if not done.urls:
        return "nothing had been edited", 0
...
def close_out(
    ...
    if not archive_due(sched, datetime.now(timezone.utc).date()) and not force:
        declared = (
