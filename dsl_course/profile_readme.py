...
| [**Archive cohort**](https://github.com/{org}/.github/actions/workflows/archive-cohort.yml) | End of term, once: revokes each student's direct access to the submission repos and gradebooks named after them, archives those repos, records what was frozen in `classroom-config/archive/teardown.md`, and archives `classroom-config` last. **Nothing is deleted** - archiving is GitHub's reversible read-only freeze. Dry run first; it refuses unless `schedule.yml`'s `semester_end` has passed. | Run by instructor |
