...
| [**Archive cohort**](https://github.com/{org}/.github/actions/workflows/archive-cohort.yml) | Close a finished cohort out early - the scheduler does it by itself on the cohort's `archive.date` (`schedule.yml`, default: `semester_end` + 60 days), after a fortnight's notice. It carries the cohort's edits back first, then archives **every repository in the cohort org** - students' work, the materials, `welcome`, the website, `.github` - records what was frozen in `classroom-config/archive/teardown.md`, and archives `classroom-config` last. **Nobody is removed and nothing is deleted**: an archived repo is read-only for everyone, so students keep read access to their own work. Dry run first; it refuses until the archive date has arrived. | Run by instructor |
