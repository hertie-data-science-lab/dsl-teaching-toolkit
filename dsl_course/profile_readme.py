...
| [**Patch released assignment**](https://github.com/{org}/.github/actions/workflows/patch-assignment.yml) | Fixed a file after the assignment went out? Commit it to the template, then run this: it pushes the file (or folder) into every submission repo as a new commit on the student's own branch - never a force-push - and posts a note on each receipts issue. A file the student has already changed is left alone unless you tick `overwrite`. Preview first. | Run by instructor |
