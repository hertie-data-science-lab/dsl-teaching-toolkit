...
# it is meant to be edited here afterwards. What the assignment IS, and nothing about how
# one semester runs it: the run settings (`settings.RUN_KEYS`) live in each semester's
# `assignments.yml`, and one written here is NOT_MIGRATED.
_GRADING_STAMP = (
    "# INSTRUCTOR-OWNED - defines the assignment. "
    "Dates live in the semester's schedule.yml."
)
_RUN_SETTINGS_NOTE = (
    "# How each semester runs it - how teams form, the max team size, the late rule, who "
    "can\n# read the repos, an external submit link - is set per semester in "
...
def _source_branches(wd: Path) -> tuple[list[str], list[str]]:
    """The branches of a clone's origin a copy CARRIES, and the ones it leaves behind -
    each in name order.
    ...
    for branch in out.split():
        if branch == "HEAD":
            continue
        toolkit_owned = branch == UPSTREAM_BRANCH or branch.startswith(
            PROPOSAL_BRANCH_PREFIX
        )
        (left if toolkit_owned else carried).append(branch)
    return carried, left


def _copy_source_head(org: str, source: str) -> str | None:
    """Which branch a copy of `org/source` must open on - or None if the copy is refused.
    ...
    Asked BEFORE anything is created, so a refusal leaves no half-made repo behind. `""`
    when GitHub would not say, which leaves the new repo's HEAD to the same guess it was
    before this pinned it - a source we could not read is not a source we know to be
    wrong."""
    head = default_branch(org, source, fallback="")
    if head == SOLUTION_BRANCH:
        log_err(
            f"  ! {org}/{source} opens on its {SOLUTION_BRANCH} branch, so a copy of it "
            "would hand the model answers out to students. Set that repo's default branch "
            "to the students' one and re-run."
        )
        return None
    return head
...
def _copy_branches(
    ...
    with tempfile.TemporaryDirectory() as work:
        ...
        if code != 0:
            ...
            return False
    # Every branch arrived in one push, and a push does not move HEAD, so until this lands
    # the new repo opens on whichever branch GitHub picked out of them. Red rather than a
    # warning: for a template, the branch it opens on is the one every student repo is
    # generated from.
    if head and not set_default_branch(org, repo, head):
        return False
    log_ok(f"copied {org}/{source} into {org}/{repo} ({', '.join(branches)})")
    if left_behind:
        log(
            f"  (left behind: {', '.join(left_behind)} - toolkit-owned working branches, "
            "rebuilt by release and propagate from whatever this repo holds)"
        )
    return True
...
def scaffold_materials(
    ...
    if copy_from and not _copy_branches(org, copy_from, repo, head=head):
        return 1
...
def _seed_template_workflows(org: str, repo: str) -> int:
    ...
    Which is also why THIS repo is named into the dropdown rather than looked for in the
    org listing: a default that is not among the options is dropped (`_assignment_input`),
    and the listing GitHub answers a moment after `create_repo` need not carry the repo it
    has only just made - or its `is_template` flag. Left to the listing, the one button
    whose whole point is this template opened on the newest OTHER assignment instead."""
...
def scaffold_assignment(
    ...
    *,
    ...
    obeys, and nothing has to be hand-edited in afterwards. How a semester runs it (the
    run settings) is that semester's `assignments.yml`, never this file.
