...
Every instant here is a run's API `created_at`, this run's own included, and never the
runner's clock: the clock at job start also carries however long this run queued, which
under Actions congestion would read a 15 min cadence as 21 and stall both close rules.
...
class Verdict:
    """What the run history says about the two drivers, as of the moment this run was
    accepted (`now`).
    ...
    # When GitHub ACCEPTED this run (its own row's `created_at`), so every interval below is
    # measured between two instants on the same clock. The runner's clock at job start is
    # the fallback, and only that - see `evaluate`.
    now: datetime
...
def evaluate(now: datetime, runs: list[dict], own_run_id: str | None) -> Verdict:
    ...
    `own_run_id` is THIS run, excluded from every "previous run" answer: it is in the
    listing (the API returns it the moment it starts) and counting it would prove every
    driver alive from inside the run that was checking - a dead-man's switch that can never
    fire. Its `created_at` is still read, and is the instant everything here is measured
    FROM: `now` (the runner's clock at job start) also carries this run's queue delay, so on
    a contended minute a 15 min cadence reads as 21 and the close rules stall exactly when
    the drivers are under most strain. `now` is the fallback for a listing that does not
    carry our own row.
    ...
    reference = own_started or now
    return Verdict(
        armed=last_dispatch is not None,
        last_dispatch_at=last_dispatch,
        last_schedule_at=last_schedule,
        ds01_dead=last_dispatch is not None and reference - last_dispatch > DS01_DEAD,
...
def late_items(
    ...
    more - the body's state marker is what stops it emailing anyone twice. `now` is this
    run's own `created_at` for the same reason: both ends of the window are then on GitHub's
    clock, and a moment that falls between it and the release itself is carried by the next
    tick's window rather than lost."""
    if prev_at is None:
        return []
    ...
    return "\n".join(
        [
            ...
            *[_item_line(item, verdict.now) for item in items],
            ...
            "",
            (
                "The gap is measured from the last tick that RAN, whether it succeeded or "
                "failed, so a spell of failing runs is reported by the `Scheduled release "
