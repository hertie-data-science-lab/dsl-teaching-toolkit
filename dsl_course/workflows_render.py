...
{_choice_input("submit_via", "8. Where students hand in. github = they push to their repo and the cutoff, receipts and late window apply; external = handed in elsewhere (Moodle, Kaggle, in class), so the repo only carries the brief and nothing is collected", list(SUBMIT_VIA_OFFERED), "github", required=False)}
      autograde:
