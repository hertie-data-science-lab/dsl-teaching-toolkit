...
{_choice_input("submit_via", "8. Where students hand in. github = they push to their repo and the cutoff, receipts and late window apply; external = handed in elsewhere (Moodle, Kaggle, in class): no repo is created, the brief and a submit link appear on the site, feedback goes to the student's gradebook; shared = one private repo for the whole cohort, each student pushes into their own folder, peers can read it", list(SUBMIT_VIA), "github", required=False)}
      autograde:
