...
def find_pr(repo: str, head: str) -> PullRequest | None:
    ...
    Raises when the listing could not be read: absence has to be a real answer. A
    listing that arrived but cannot be MADE SENSE OF is the same answer, so the rows are
    sifted inside the same `try` - a payload that is not a list, or a row with no
    `number`, is a KeyError or a TypeError, and outside it neither is a RuntimeError.
    `upsert_pr` catches only that, so either one went straight past it and out of
    `deploy_many`, taking every other dest's release in the batch with it.
