...
| `enrolment` | `send_codes_datetime` (**required**), `send_until` (default: `semester_start` + 2 weeks), `show_on_site` (default **false**), `title` | `enrol_codes.run` on every tick inside the window - idempotent on `code_sent_at` |
