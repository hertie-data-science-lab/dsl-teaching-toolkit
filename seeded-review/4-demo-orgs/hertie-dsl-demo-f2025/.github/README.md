<!-- source: https://github.com/hertie-dsl-demo-f2025/.github/blob/main/README.md @ fa5e9c3 -->
<!-- SYSTEM-OWNED - do not edit, edits here are overwritten on the next refresh. -->

# Deep Learning (Demo) - cohort control repo

This is the **`.github` repo** for the `hertie-dsl-demo-f2025` cohort org. **Students and instructors rarely need to touch anything in this repo directly.**

Built and kept in sync by the [DSL teaching toolkit](https://github.com/hertie-data-science-lab/dsl-teaching-toolkit).
