...
def test_people_yaml_semester_includes_tas():
    # The semester site reads its own people.yml and renders instructors AND TAs.
    out = site_repo.people_yaml(
        "Some-Semester-f2026", _PEOPLE_META, edit_at="people.yml"
    )
...
def test_people_yaml_header_says_it_is_generated_and_where_to_edit(monkeypatch):
    ...
    declared = site_repo.people_yaml(
        "Some-Semester-f2026",
        _PEOPLE_META,
        edit_at="Semester/classroom-config/people.yml",
    )
    ...
    assert "Semester/classroom-config/people.yml" in declared
...
def test_people_yaml_passes_every_declared_display_field_through():
    ...
    out = site_repo.people_yaml("Some-Semester-f2026", meta, edit_at="people.yml")
