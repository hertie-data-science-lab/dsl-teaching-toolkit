...
def test_this_runs_own_queue_delay_is_not_counted_as_a_late_driver():
    # The runner's clock starts when the JOB does, so it carries however long this run sat
    # in the Actions queue. Six minutes of queue would read a punctual 15 min cadence as 21
    # - past HEALTHY_GAP - and stall both close rules exactly when the estate is contended.
    queued = NOW - timedelta(minutes=6)  # when GitHub accepted this run
    runs = [
        _run(6, run_id=OWN),  # our own row, at `queued`
        _run(21, run_id=1),  # the previous dispatch, 15 min before that
    ]
    v = cadence.evaluate(NOW, runs, OWN)
    assert v.now == queued
    assert v.now - v.last_dispatch_at == timedelta(minutes=15)
    assert v.dispatch_on_cadence is True


def test_a_long_queue_does_not_invent_a_dead_dispatcher():
    # The other end of the same bug: an Actions incident that queued this run for two hours
    # would open a driver-health issue about a dispatcher that never missed a fire.
    runs = [_run(115, run_id=OWN), _run(130, run_id=1)]
    assert cadence.evaluate(NOW, runs, OWN).ds01_dead is False
    # ...whereas measured off the clock the same listing reads as dead
    assert _evaluate([_run(130, run_id=1)]).ds01_dead is True


def test_without_our_own_row_the_clock_is_the_fallback():
    # A listing that does not carry us (a local invocation, a window that has scrolled past
    # this run) still has to answer; the runner's clock is the only instant left.
    v = _evaluate([_run(21, run_id=1)])
    assert v.now == NOW
    assert v.dispatch_on_cadence is False
