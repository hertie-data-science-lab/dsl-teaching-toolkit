...
def test_a_listing_nothing_can_be_made_of_opens_nothing_either(gh_pr, capsys):
    # Same contract, from the other side: the sift used to sit OUTSIDE the try, so a row
    # with no `number` raised a KeyError - not a RuntimeError - past `upsert_pr`'s catch
    # and out of `deploy_many`, taking every other dest's release with it.
    fake = gh_pr([{"url": "u", "headRefName": HEAD}])
    with pytest.raises(RuntimeError):
        pulls.find_pr(REPO, HEAD)
    assert _upsert() == pulls.Upserted(1, None)
    assert fake.did("pr", "create") == []
    ...
    # And a payload that is not a listing at all - a TypeError, for the same reason.
    fake.rows = "not a listing"
    assert _upsert() == pulls.Upserted(1, None)
    assert fake.did("pr", "create") == []
