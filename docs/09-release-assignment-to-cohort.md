# Manually release an assignment to a semester
...
Auditors (`role=auditor`) are skipped. The assignment's brief appears on the semester site automatically - at hand-out, not before, however you hand out.
...
**Allow members to delete or transfer repositories** OFF. The semester's *grading_config.yml*
...
on that declaration, so a handout free to disagree with it would put a semester's work in
...
`semester-config/snapshots/<slug>.csv`. It is **write-once** - later pushes can't move the
