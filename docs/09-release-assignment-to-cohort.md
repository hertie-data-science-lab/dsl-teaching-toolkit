...
**The team list.** `teams.csv` is private, so a student cannot see what to join. The tick that opens the window opens a pinned **Teams for `<assignment>`** issue in the cohort's `welcome` repo, and every successful join rewrites it: the cap, the closing day, and one line per team with its `n/cap` and whether it has room - **names and counts only**, because that repo is public. The mail, the form's header and the assignment's page all link it. One issue per assignment, rebuilt from the file each time, so it needs no upkeep; leave it open.

**The cohort site** says the same where students already are. While the window is open, that assignment's page carries a **Form your team** callout - the cap, the closing day, a link to the form, and a table of the teams that exist with the places each has left (names and counts only) beside a link to the list issue - and its schedule row carries the same prompt in one line. A push to `teams.csv` fires a site sync, so the table follows each join within minutes. The submission address stays beside it rather than in its place: a team that formed on day one owns its repo already. All of it goes when the window shuts.

**The students are emailed** - the only mail the toolkit sends off a clock rather than off something you did. When the window opens, every enrolled, onboarded student with no team for that assignment gets one plain message, addressed to them by name and naming the assignment as the site does: the cap, the day formation closes, the link to the form and the link to the team list. Anyone still without a team 48 hours before it shuts gets one more, and a student who onboards mid-window is asked on the next tick. Nothing goes out between 23:00 and 07:00 in the cohort's timezone - the message waits for the morning, while the lock, the form and the site still turn at the window's own minute - nothing is said twice (`team-formation/mailed.csv` in `classroom-config` is the record), and a cohort whose course org has no `GRAPH_*` mail secrets is sent nothing and has nothing recorded against it. It does not say whether working alone is allowed - that is yours to tell them.
