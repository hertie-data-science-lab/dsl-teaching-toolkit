...
**You hear about it too.** While the window is open and anybody is still without a team, the cohort's *schedule.yml* [digest issue](07-schedule-releases.md#the-digest-issue) carries a line on that assignment - counts only, never a name - and it gets louder as the window runs out, exactly like a source nobody has staged. Fill the gaps in `teams.csv` yourself, or move the closing date to give the cohort longer.
