...
| **New assignment** | Scaffold an `assignment-N-<tag>` template from nine answers (name, number, tag, format, type, team_formation, submit_via, autograde, copy_from): brief + starter (+ `CONTRIBUTIONS.md` for a group) on `main`; stub solution and `grading_config.yml` on the `solution` branch, with a hidden-test stub only when autograding was asked for. `format` picks the starter stub and nothing else; `type: group` makes handout + grading run per team. `copy_from` starts it as an existing template instead - every branch, the whole history - and the name and boxes 4-8 are then ignored ([03](../03-add-assignment-to-course.md)). The template gets its own **Release assignment** button, pre-selected on itself. |
