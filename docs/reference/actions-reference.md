...
| **New assignment** | Scaffold an `assignment-N-<tag>` template from nine answers (name, number, tag, format, type, team_formation, submit_via, autograde, copy_from): brief + starter (+ `CONTRIBUTIONS.md` for a group) on `main`; stub solution and `grading_config.yml` on the `solution` branch, with a hidden-test stub only when autograding was asked for. `format` picks the starter stub and nothing else; `type: group` makes handout + grading run per team. `copy_from` copies an existing template forward - `main` and `solution` whole, history included - and then boxes 4-8 are ignored, because the copied `grading_config.yml` is the definition; the source must have a `solution` branch. |
