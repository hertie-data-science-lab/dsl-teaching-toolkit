...
| Action | Effect |
| --- | --- |
| **Bootstrap semester** | Configure a pre-created semester org: `welcome` + `classroom-config`, permissions, site, `course_admins`, register + refresh. Safe to re-run on a live semester - your `classroom-config` files are never overwritten. |
...
The first six live in that semester's private `classroom-config` and cc its `instructors`; the
...
- *"Scheduled release: late delivery"*, in the semester's private `classroom-config`, cc that
