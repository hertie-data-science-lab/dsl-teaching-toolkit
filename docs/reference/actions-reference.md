...
| **Send enrolment codes** | Generate an `enrol_code` per roster row, write it back to `students.csv`, email each not-yet-onboarded student theirs. Safe to re-run: a row's `code_sent_at` stops it being mailed twice (from the first run that sets it). Automatic on a push to a cohort's `students.csv`, and that path sends for real - so the button is the fallback. **`dry_run` defaults to `true` - nothing is written or sent until you untick it.** |
