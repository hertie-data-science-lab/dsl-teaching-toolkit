...
| **New assignment** | Scaffold an `assignment-N-<tag>` template from nine answers (name, number, tag, copy_from, format, type, team_formation, submit_via, autograde): brief + starters (+ `CONTRIBUTIONS.md` for a group) on `main`; stub solutions and `grading_config.yml` on the `solution` branch, with a hidden-test stub only when autograding was asked for. `format` takes a comma-separated list and picks the starter stubs and nothing else; `type: group` makes handout + grading run per team. `copy_from` starts it as an existing template instead - every branch, the whole history - and boxes 5-9 are then ignored ([03](../03-add-assignment-to-course.md)). The template gets its own **Release assignment** button, pre-selected on itself. |
