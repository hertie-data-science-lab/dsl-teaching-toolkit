...
| semester site rows keyed by the `NN_` folder ordinal; the `readings` section; `readings_pending` | one row per `releases:` entry, of its kind (`schedule_plan.planned_rows`); tabs per kind; the pinned syllabus by `materials.yml` declaration | every semester site, on its next sync |
