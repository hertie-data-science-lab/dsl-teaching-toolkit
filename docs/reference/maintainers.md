...
until faculty settle it. The config commit is compare-and-set (`put_files(base=...)`): a
write that landed after the read fails it, and the next sync retries. The old account then
