...
> Enrolment-code + grade emails send as `datasciencelab@hertie-school.org` via a central Entra app, authenticated by certificate ([details](../docs-admin-arch/central-admin.md#email)). Live on every course org except `hertie-nlp-e1282`. Where it isn't live, enrolment codes still land in `students.csv` (from which they need to be manually emailed), and grades are still sent to students' grades repos (just the notification email is not sent).
