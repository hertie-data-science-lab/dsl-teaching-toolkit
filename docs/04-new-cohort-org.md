# New semester org (once per year)
...
7. *(only for `visibility: student_choice` assignments)* **Set the org's member privileges.**
    ...
    - The semester's *grading_config.yml* digest issue reports it while either is wrong.
...
- [Release ad hoc](08-release-materials-to-cohort.md), if you want something out before the
  schedule says so.
- [Configure the semester website](11-configure-cohort-site.md) - the site step 3 just seeded.
