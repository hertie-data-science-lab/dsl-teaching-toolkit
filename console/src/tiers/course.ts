...
export const ABOUT: Tiers = {
  course_name: { tier: 'ask', label: 'Course name', reason: 'Appears on every cohort’s student site.' },
  course_code: { tier: 'ask', label: 'Course code', reason: 'Upper case here; lower case in the org name.' },
  course_description: { tier: 'default', label: 'Description', widget: 'markdown', reason: 'One paragraph. Students see it on every cohort’s student site home page.' },
...
export const COHORT_DEFAULTS: Tiers = {
  timezone: { tier: 'default', label: 'Timezone', widget: 'select', options: [opt('', 'Europe/Berlin (the toolkit’s default)'), ...TIMEZONES.map((t) => opt(t, t))], reason: 'Written into each new cohort’s schedule.' },
  archive_auto: { tier: 'default', label: 'Archive automatically after term ends', widget: 'checkbox', default: true },
  grace_days: { tier: 'conditional', under: 'archive_auto', when: (v: Values) => v.archive_auto !== false, label: 'Days after term end', widget: 'number', placeholder: '60' },
};
