...
describe('the live demo files', () => {
  ...
  it('saves course details over card dates and course-level assistants, with a note', () => {
    ...
    expect('warning' in out && out.warning).toBe(
      'Not checked: dsl-course.yml has course-level teaching assistants and dates on a course instructor card, which the engine reads but the console’s check does not know yet.',
    );
  });
